...
namespace :app do
  ...
  task :configure_sessions, [:config_file] => :environment do |_, args|
    ...
    CSV.foreach(args[:config_file], headers: true) do |row|
      ...
      if constraints.blank?
        ...
        if !presenter
          presenter = session.presenters.find { |p| p.sessions_presenting.where(event: event).count == 1 }
          if !presenter
  ...
  task analyze_scheduler_input_quality: :environment do
    ...
    multivoting_count = vote_counts.count { |_, count| count > 1 }
    ...
    frequency_dump(vote_counts.values) do |vote_count, freq|
      if freq == 1
        participant_id = vote_counts.find { |_, count| vote_count == count }.first
